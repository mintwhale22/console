...
const OwnerList = () => {
    ...
    const gotoEdit = (seq) => {
        navigate("/owner/edit/" + seq.toString());
    }
                                ...
                                <CTableRow>
                                    ...
                                    list.map((member, index) => (
                                        <CTableRow valign="middle">
                                            ...
                                            <CTableDataCell
                                                className="text-center">{member.store.length.toString()}상점</CTableDataCell>
                        ...
                        <CPagination align="center">
                            {
                                ypaging.max > pageLangth && (
                                    <CPaginationItem onClick={() => { gotoPage(1) }}>
                            ...
                            {
                                ypaging.max > pageLangth && (
                                    <CPaginationItem onClick={() => { gotoPage(ypaging.prev) }} disabled={page <= pageLangth}>
                            ...
                            }
                            {
                                Array((ypaging.end - ypaging.start + 1)).fill().map((_, index) => (
                                    <CPaginationItem onClick={() => { gotoPage(ypaging.start + index) }} active={(ypaging.start + index).toString() === page.toString()}>
                                        {ypaging.start + index}
                                    </CPaginationItem>
                                ))
                            }
                            {
                                ypaging.max > pageLangth && (
                                    <CPaginationItem onClick={() => { gotoPage(ypaging.next) }} disabled={page >= (ypaging.max - pageLangth)}>
                            ...
                            {
                                ypaging.max > pageLangth && (
                                    <CPaginationItem onClick={() => { gotoPage(ypaging.max) }}>
