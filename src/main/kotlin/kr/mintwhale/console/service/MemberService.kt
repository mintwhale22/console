...
class MemberService {
    ...
    fun edit(data: Member): Boolean? {
        ...
        if (data.intType == 2) {
            val listSearch = ListSearch()
            val delstore = Store()
            delstore.intMSeq = data.intSeq
            listSearch.search = delstore

            val result4 = storeMapper.getStore(listSearch)
            for (store in result4) {
                delstore.intSeq = store.intSeq
                delstore.intStatus = 99
                storeMapper.editStore(delstore)
            }
            ...
            delstore.intSeq = null
            delstore.intStatus = 99
            storeMapper.delStore(delstore)
        }
    ...
    fun add(data: Member): Boolean? {
        ...
        if (data.intType == 2 && result && data.arrStore != null) {
            for (field in data.arrStore!!) {
                field.intMSeq = data.intSeq
